...
export async function load(handler: tf.io.IOHandler, config: ModelConfig = {}) {
  if (tf == null) {
    throw new Error(
  ...
  await objectDetection.load(handler);
...
export class ObjectDetection {
   ...
   * of detected objects. Value between 0 and 1. Defaults to 0.5.
   */
  async detect(img: tf.Tensor3D, maxNumBoxes = 20, minScore = 0.5): Promise<DetectedObject[]> {
