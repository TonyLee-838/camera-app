...
function validateModelConfig(config: ModelConfig) {
  ...
  if (VALID_ARCHITECTURE.indexOf(config.architecture) < 0) {
    throw new Error(
      `Invalid architecture ${config.architecture}. ` + `Should be one of ${VALID_ARCHITECTURE}`
    );
  }
  ...
  if (config.architecture === "MobileNetV1" && config.outputStride === 32 && config.multiplier !== 1) {
...
function validateSinglePersonInferenceConfig(config: SinglePersonInterfaceConfig) {}
...
async function loadMobileNet(handler: tf.io.IOHandler, config: ModelConfig): Promise<PoseNet> {
  ...
  const graphModel = await tf.loadGraphModel(handler || config.modelUrl || url);
...
async function loadResNet(handler: tf.io.IOHandler, config: ModelConfig): Promise<PoseNet> {
  const outputStride = config.outputStride;
  const quantBytes = config.quantBytes;
  if (tf == null) {
    throw new Error(
...
export async function load(
  handler: tf.io.IOHandler,
  config: ModelConfig = MOBILENET_V1_CONFIG
): Promise<PoseNet> {
  config = validateModelConfig(config);
  if (config.architecture === "ResNet50") {
    return loadResNet(handler, config);
  } else if (config.architecture === "MobileNetV1") {
    return loadMobileNet(handler, config);
  } else {
    return null;
  }
}
