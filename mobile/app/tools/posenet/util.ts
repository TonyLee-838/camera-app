...
export async function toTensorBuffers3D(tensors: tf.Tensor3D[]): Promise<TensorBuffer3D[]> {
  return Promise.all(tensors.map((tensor) => tensor.buffer()));
}

export function scalePose(pose: Pose, scaleY: number, scaleX: number, offsetY = 0, offsetX = 0): Pose {
  return {
    score: pose.score,
...
export function scalePoses(poses: Pose[], scaleY: number, scaleX: number, offsetY = 0, offsetX = 0) {
...
function isValidInputResolution(resolution: number, outputStride: number): boolean {
...
export function assertValidResolution(resolution: [number, number], outputStride: number) {
  ...
  tf.util.assert(
    ...
    () => `height of ${resolution[0]} is invalid for output stride ` + `${outputStride}.`
  );

  tf.util.assert(
    isValidInputResolution(resolution[1], outputStride),
    () => `width of ${resolution[1]} is invalid for output stride ` + `${outputStride}.`
  );
}

export function getInputTensorDimensions(input: PosenetInput): [number, number] {
...
export function padAndResizeTo(
  ...
  const resized: tf.Tensor3D = tf.tidy(() => {
    ...
    imageTensor = tf.pad3d(imageTensor, [
      [padT, padB],
      [padL, padR],
      [0, 0],
    ]);
